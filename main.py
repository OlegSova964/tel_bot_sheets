...
def extract_meta(text: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for ln in lines:
        ...
        # ===== first column candidates (creo / file name) =====
        if ":" in ln:
            if ("название крео" in low) or low.startswith("крео:") or low.startswith("creo:") or ("название файла" in low):
                v = ln.split(":", 1)[1].strip()
                if v:
                    creo = v

            # ===== second column candidates (name / card) =====
            if low.startswith("название:") or low.startswith("name:") or ("название карточки" in low):
                v = ln.split(":", 1)[1].strip()
                if v:
                    nm = v
    ...
    while True:
        ...
        try:
            async with sheet_lock:
                # ===== 3 колонки =====
                # A: file (если есть) ИНАЧЕ creo
                # B: name / card
                # C: tag
                first_col = file_value if file_value else creo_value
                append_row_with_retry(sheet, [first_col, name_value, tag])
...
async def flush_album(media_group_id: str):
    ...
    for fname in items:
        ...
        futs.append(await enqueue_write(user_id, chat_id, clean, creo or "", name, tag))

    await asyncio.gather(*futs)
    await bot.send_message(chat_id, "Готово! Жду следующую порцию.")
...
async def handle_message(message: Message):
    ...
    if not is_any_file(message):
        ...
        fut = await enqueue_write(user_id, chat_id, "", creo or "", name, tag)
        await message.answer("✅ Принято (текст). Записываю...")
        await fut
        await message.answer("Готово! Жду следующую порцию.")
        return

    # 2) Если пришёл файл — для записи нужна 'Название:' (и опционально крео)
    if not name:
    ...
    clean = strip_ext(fname)
    fut = await enqueue_write(user_id, chat_id, clean, creo or "", name, tag)
